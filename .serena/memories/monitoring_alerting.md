...
`~/watchdog/trade_report.py` (ex `hl_report.py`, rinominato 2026-09-15) su amazon, cron **`*/5 * * * *`**, log `~/watchdog/hl_report.log`, stato `~/watchdog/trades_state.json`. Copia versionata in **`ops/hl_report.py`**. **Sola lettura**: non riavvia e non tocca il bot — guasti e riavvii restano esclusiva del watchdog. È l'unico canale con invio Telegram diretto attivo (`send()` propria, con `~/watchdog/telegram.json`).
