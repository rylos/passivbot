...
### È la manutenzione settimanale di Hyperliquid (verificato il 2026-09-21)
Intuizione di Marco, confermata sui 21 log di ry-hl: **9 episodi su 9 cadono di sabato o domenica**, cadenza settimanale, sempre fra le 07:35 e le 09:29 UTC (09:35-11:29 Rome) — 25/07, 01/08, 08/08, 15/08, 23/08, 29/08, 05/09, 13/09, 19/09. Ogni raffica dura **~30 secondi** e finisce **sempre** con `restart_at_limit`. Su ry-bybit non succede mai: è solo lato Hyperliquid.
**Non è un guasto e non va corretto**: il bot si riprende da solo. Un `up=` azzerato il weekend mattina su hl si spiega così, senza riaprire l'indagine. Se un giorno comparisse **di feriale** o durasse **molto più di 30 s**, allora è un'altra cosa.

## Notifiche trade su Telegram (dal 2026-08-15; **event-driven dal 2026-08-27**)
`~/watchdog/trade_report.py` (ex `hl_report.py`, rinominato 2026-09-15) su amazon, cron **`*/2 * * * *`** (era `*/5`, sceso a 2 min il 2026-09-16: script in sola lettura, <1 s a giro, cosi' una chiusura si notifica entro 2 min invece di 5; sotto il minuto servirebbe un `flock`, due giri sovrapposti scriverebbero lo stesso stato), log `~/watchdog/trade_report_hl.log` (bybit: `trade_report_bybit.log`), stato `~/watchdog/trades_state.json` (bybit: `trades_state_bybit.json`). Copia versionata in **`ops/trade_report.py`**. **Sola lettura**: non riavvia e non tocca il bot — guasti e riavvii restano esclusiva del watchdog. È l'unico canale con invio Telegram diretto attivo (`send()` propria, con `~/watchdog/telegram.json`).
...
2. **Se `last_key` esce dal buffer letto dal log** (rotazione, o script fermo a lungo) NON si rigioca lo storico: ci si **risincronizza in silenzio**. Rigiocare manderebbe una raffica di notifiche su trade vecchi. Meglio perdere una notifica che inondare la chat — i trade restano nel log. **Eccezione dal 2026-09-15**: log nuovo dopo riavvio bot con ≤10 eventi tutti successivi a `last_key` → si mandano (vedi sezione in fondo).
...
`~/watchdog/watchdog.py` e `~/watchdog/trade_report.py` su amazon scelgono l'istanza dal primo argomento (`bybit`), default `hl` → il cron storico di ry-hl è invariato. Per istanza: stato (`state.json`/`state_bybit.json`, `trades_state.json`/`trades_state_bybit.json`), flag di manutenzione (`hl_maintenance`/`bybit_maintenance`), tmux, log alias (`hyperliquid_vault.log`/`bybit_02.log` = `<user>.log` creato da passivbot), check healthchecks (`HC_PASSIVBOT`/`HC_PASSIVBOT_BYBIT`; il secondo è l'ex check freqtrade rinominato "passivbot (bybit)" via API il 2026-09-03, stesso uuid). Cron ry-bybit: `3-59/10` watchdog, `*/2` report. Il watchdog freqtrade e `ft_report.py` sono commentati nel crontab (freqtrade fermo). Copie versionate: `ops/watchdog_ry_hl.py`, `ops/hl_report.py`, `ops/README.md`.
...
## hl_report.py: inoltro al gruppo RyLoS-Trading (2026-09-10)
Su richiesta di Marco, le notifiche del profilo **hl** (ry-hl) vanno anche al supergruppo Telegram **RyLoS-Trading** (chat_id −1001330952946) tramite il bot **@freqtradehl_bot** (lo stesso usato da freqtrade-hl). Implementazione: `PROFILES[...]["extra_creds"]` (lista di file in `~/watchdog/`; hl → `telegram_rylos_group.json`, bybit → vuota), `send()` cicla su `[telegram.json] + extra` e un errore su un destinatario non blocca gli altri. Credenziali in `~/watchdog/telegram_rylos_group.json` (chmod 600, copiate da `/opt/freqtrade-hl/user_data/config.json`, mai nel repo). Backup `hl_report.py.bak-20260910`, repo `ops/hl_report.py` allineato (commit 2d9ed671e). Il canale principale verso Marco resta Claude RyLoS Bot.

## hl_report: pnl netto e finestra sull'orario del fill (2026-09-14, commit 34b7f5f43)
- Il bot (entrambe le istanze, deploy 2026-09-14 20:23/20:25 Rome da flat) scrive `fee=<cashflow con segno>` su ogni riga `[fill]`, ingressi compresi. `hl_report.py` somma pnl + fee di tutti i fill del ciclo e manda il **netto**, uguale al realized pnl dell'exchange (14/09: lordo +15,37, Bybit +14,76). Con log senza `fee=` resta il lordo.
- La riga `[fill]` può comparire 40-50 s dopo la riga `[pos] closed`: la finestra usa l'orario proprio del fill (non quello della riga di log) e, se alla chiusura non c'è ancora nessun fill nel log, l'evento resta in sospeso fino al giro successivo (last_key non avanza). Prima usciva "+0.00".
